...
function ConversorDeMoedas() {
    ...
    useEffect(()=> {
        fetch('http://api.exchangeratesapi.io/v1/latest?access_key=675de778b39d951e97af28198ee9da14&format=1') 
    ...
    fetch(`http://api.exchangeratesapi.io/v1/latest?access_key=675de778b39d951e97af28198ee9da14&format=1base=${deMoeda}&symbols=${paraMoeda}`) 
        .then(response => response.json())
        .then(data => {
            const taxaDeCambio = data.rates[paraMoeda]; 
            setResultado(quantidade * taxaDeCambio);
        })
        .catch(error => console.error('Erro ao converter moeda:',));
    }; 
    return (
        <div> 
